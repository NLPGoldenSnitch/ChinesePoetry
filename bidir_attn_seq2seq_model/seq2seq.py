...
def bidirectional_attention_seq2seq(encoder_inputs,
                                    decoder_inputs,
                                    cell,
                                    num_encoder_symbols,
                                    num_decoder_symbols,
  ...
  with variable_scope.variable_scope(
    ...
    embedded_inputs = [embedding_ops.embedding_lookup(embedding, b) for b in encoder_inputs]
    #encoder_cell = core_rnn_cell.EmbeddingWrapper( cell, embedding_classes=num_encoder_symbols, embedding_size=embedding_size)
    enc_cell_fw = cell(embedding_size)
    enc_cell_bw = cell(embedding_size)
    #encoder_outputs, encoder_state = core_rnn.static_rnn( encoder_cell, encoder_inputs, dtype=dtype) 
    encoder_outputs, *encoder_states = core_rnn.static_bidirectional_rnn(
                                         enc_cell_fw,
                                         enc_cell_bw,
                                         embedded_inputs,
                                         dtype = dtype
    ...
    top_states = [
        array_ops.reshape(e, [-1, 1, embedding_size * 2]) for e in encoder_outputs
    ]
    attention_states = array_ops.concat(top_states, 1)

    # Decoder.
    dec_cell = cell(embedding_size)
    output_size = None
    if output_projection is None:
      dec_cell = core_rnn_cell.OutputProjectionWrapper(dec_cell, num_decoder_symbols)
      output_size = num_decoder_symbols

    encoder_state = array_ops.concat(encoder_states, 1)
    enc2dec_w = variable_scope.get_variable("enc2dec_w", [embedding_size * 2, embedding_size])
    enc2dec_b = variable_scope.get_variable("enc2dec_b", [embedding_size])
    decoder_init_state = math_ops.matmul(encoder_state, enc2dec_w) + enc2dec_b
    if isinstance(feed_previous, bool):
