...
def run(isGen):
    file_poem = "data/q7.from"
    file_target = "data/q7.to"
    file_keyword = "data/q7.key"
    args = ControlParm()
    data = DataHandle(file_poem,file_keyword,file_target, args)
    ...
    #training mode
        ...
        with tf.Session() as sess:
            ...
            for i in range(args.begin_iter+1,batch_times):
                learning_rate = args.learning_rate * (args.decay_rate ** 
                    (i // args.decay_steps))
                if learning_rate <= 0.000000000000001:
