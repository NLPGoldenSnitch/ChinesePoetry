    ...
    def __init__(self,args,data,isGen,model='gru'):
        ...
        with tf.variable_scope('decoder_variable') as decoder_scope:
            for i in range(s_len):
                ...
                if isGen == 1:
                    ch_logit = tf.matmul(decoder_outputs[i],self.proj_w)+self.proj_b
                    ch_probs = tf.nn.softmax(ch_logit)
                    ch_id =tf.argmax(ch_probs,axis=1)
                    self.generate_outputid.append(ch_id)
                    embedded_output_id = tf.nn.embedding_lookup(self.embeddings, ch_id)
